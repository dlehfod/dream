// Supabase Edge Function - 꿈 분석 API
// Gemini API 키를 서버 측에서 안전하게 관리합니다.
...
// 프롬프트 생성 함수
function buildPrompt(dreamData: { story: string; symbols: string; emotion: string }) {
...
serve(async (req) => {
  ...
  try {
    ...
    // 요청 데이터 파싱
    const dreamData = await req.json()
    
    if (!dreamData.story && !dreamData.symbols && !dreamData.emotion) {
      throw new Error('꿈 정보가 제공되지 않았습니다.')
    }
